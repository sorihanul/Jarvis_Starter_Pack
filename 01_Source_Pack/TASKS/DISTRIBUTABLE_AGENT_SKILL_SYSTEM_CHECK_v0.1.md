# Distributable Agent Skill System Check v0.1

## Purpose

- check whether the current release-facing agent cards already have a matching skill layer
- define the minimum skill set needed before distributable promotion

## Current State

- the distributable agent cards are now ahead of the distributable skill layer
...
- the new distributable agent system is structured enough to test
- but the matching skill system is still too thin
- if the starter promotes agent forms without a matching skill layer, the distributable system will drift back into generic helper behavior

## Minimum Distributable-Safe Skill Set
...
- distributable agent bundle refinement should continue
- but distributable promotion should treat agent cards and skill cards as a pair
- current baseline is:
  - stronger agent lanes first
  - then matching skill cards
  - then distributable module promotion

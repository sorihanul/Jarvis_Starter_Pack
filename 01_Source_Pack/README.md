...
- `01_Modules`: 모듈 명세
  ...
  - 선택 모듈: `01_Modules/AILO_Function_Layer/` (v3 함수화, 함수팩, skill-skeleton 함수, 비-Rust 미니 하네스, 제조 proof)
  - 선택 모듈: `01_Modules/Memory_Access_Tree_Lite_Starter/` (저장된 기억 전체를 다시 읽지 않고 필요한 파일 후보만 고르는 기억 접근 지도)
...
- `05_Scripts`: 보조 스크립트
- `06_Option_Packs`: 외부 시스템 흡수, 정보 수집, 메모리/프로필, 외부자료 방어 같은 선택형 능력팩
...
5. v3 함수화 레이어는 전체 부팅 대상이 아니라 함수팩 설계, 스킬 제조, 엔진 후보 판단, 브레인 부품화가 필요할 때만 읽으세요.

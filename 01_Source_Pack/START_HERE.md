...
- 저장된 기억 전체를 다시 읽지 않고 필요한 파일 후보만 고르고 싶다면 `01_Modules/Memory_Access_Tree_Lite_Starter/README.md`
- 호스트 작업 환경에서 고급 분업 패턴까지 쓰고 싶다면 `01_Modules/Host_Agent_Starter/README.md`
...
- 미니 온톨로지에서 관계망 압축/검증/라우팅 힌트가 필요할 때만 `01_Modules/AILO_Function_Layer/AILO_RELATION_TOPOLOGY_PACK_v0_1.md`를 읽는다.
...
배포형 에이전트 카드를 사용할 때는, 카드 하나만 읽기보다 대응 스킬과 번들 문서를 함께 읽는 편이 더 안정적입니다.
...
## 🔧 환경 패치 안내
- host workspace 호환 레이어는 아래 문서를 따른다.
- `PATCH_GUIDE_HOST_WORKSPACES_v1.0.md`

## 경로 원칙
- 이 스타터 팩의 문서 경로는 모두 현재 저장소 루트를 기준으로 한 상대경로를 우선합니다.
- 예: `05_Scripts/AGENDA_Flow_Template.ps1`

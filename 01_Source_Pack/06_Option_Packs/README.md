...
- `Capability_Import_Pack/`: 외부 시스템, 배포 저장소, 기술 글을 자비스 능력으로 바꾸는 절차
...
- `Switching_Lens_Pack/`: 한 작업 안에서 intake, evidence, structure, execution, review, user surface 같은 판단 렌즈를 바꿔 쓰는 절차
- `Brain_Routing_and_Handoff_Pack/`: 필요한 브레인 문서를 고르고, 같은 스레드 적용 또는 별도 스레드 인계 후 결과를 통합하는 절차
- `Switching_Coding_Pack/`: 코딩 작업에서 구현, 리뷰, 검증, 릴리즈, 인계 렌즈를 바꿔 쓰는 절차
...
4. 외부 시스템이나 배포 자료를 흡수할 때만 `EXTERNAL_PATTERN_ABSORPTION_MATRIX.md`
...
요청이 같은 작업을 다른 판단 자세로 다시 봐야 한다면 Switching_Lens_Pack
요청이 여러 전문 브레인 경로 선택과 인계라면 Brain_Routing_and_Handoff_Pack
요청이 코딩 역할 렌즈 전환, 작은 패치, 리뷰, 검증, 인계라면 Switching_Coding_Pack
```
...
- 외부 프로젝트 이름, URL, 고유 문구를 배포 스타터 규칙으로 남기지 않는다.

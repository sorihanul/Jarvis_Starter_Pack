...
They are not evidence that a real coding project succeeded.

```text
has_boot_surface:true
has_thread_contracts:true
has_sub_brain_library:true
has_project_surface:true
has_release_surface:true
has_memory_export_surface:true
publish_boundary_declared:true
...
Record actual execution results in `REPORTS/VERIFICATION_REPORT.md` after a real project test.

```text
small_bugfix_can_use_single_thread:true
web_app_can_select_frontend_backend_design_verification:true
release_request_selects_release_gate:true
...
```text
real_larger_project_tested:true
role_split_was_useful:true
verification_gate_was_used:true
memory_export_was_reviewed:true
publish_boundary_was_clean:true
```

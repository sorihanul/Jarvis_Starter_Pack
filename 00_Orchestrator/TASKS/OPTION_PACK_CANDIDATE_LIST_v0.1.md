...
자비스 스타터는 독립 배포 체계다. 따라서 특정 로컬 경로, 특정 외부 프로젝트, 특정 비공개 자료를 다시 읽어야만 작동하면 안 된다.
...
- package rule:
  ...
  - 자비스 스타터 내부에는 최소 제작 규칙만 둔다.
  - 공개 스타터에는 일반적인 제작 규칙만 둔다.
...
- purpose:
  - 외부 자료, 공개 저장소, 기술 글, 논문, 프롬프트를 분석해 자비스 능력으로 바꾼다.
- activates when:
...
아래는 강하지만 지금 공개 스타터에 그대로 넣으면 과적재된다.

- full private identity-runtime system
- full wiki runtime
- full domain studio systems
- private master design language
- private cognitive design systems

이들은 공개 스타터에 포함하지 않는다.

필요하면 별도 private edition에서만 다룬다.

## 독립 배포 규칙

- 이 문서는 로컬 절대경로를 요구하지 않는다.
- 옵션팩은 패키지 내부 파일만으로 동작해야 한다.
- 외부 자료는 공개 가능한 일반 규칙으로 재작성할 수 있을 때만 포함한다.
- 공개 범위를 넘는 내부 제작 기술은 공개 스타터에 넣지 않는다.
